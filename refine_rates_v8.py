...
def refine_data():
    print(f"--- Starting Data Refinement (v8) on {INPUT_FILE} ---")
    ...
    try:
        with open(INPUT_FILE, mode='r', encoding='utf-8-sig', errors='replace') as f:
            ...
            for i, row in enumerate(reader):
                # We look for specific known columns to confirm this is the header
            ...
            # Reset file pointer and skip to data
            f.seek(0)
            # Skip the junk rows + the header row itself to get to data
            for _ in range(header_row_index + 1):
                next(f)
            
            # Now read the data using the headers we found
